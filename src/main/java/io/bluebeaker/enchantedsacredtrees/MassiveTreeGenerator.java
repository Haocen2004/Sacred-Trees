...
public class MassiveTreeGenerator {
    ...
    private void genVines(Level level, int x, int y, int z, int length) {
        for (int i = 1; i < length; i++) {
            if (!level.getBlockState(new BlockPos(x, y - i, z)).isAir()) return;
            this.setBlockAndNotifyAdequately(level, x, y - i, z, vines);
        }
        if (vines2 != null && level.getBlockState(new BlockPos(x, y - length, z)).isAir())
            this.setBlockAndNotifyAdequately(level, x, y - length, z, vines2);
    }

    private boolean canReplaceWithLeaves(BlockState state, LevelAccessor level, BlockPos pos) {
        return state.isAir() || state.is(BlockTags.LEAVES) || state.is(BlockTags.SAPLINGS) || state.canBeReplaced();
    }

    private boolean canReplaceWithLogs(BlockState state, LevelAccessor level, BlockPos pos) {
        return state.isAir() || state.is(BlockTags.LEAVES) || state.is(BlockTags.SAPLINGS) ||
                state.is(BlockTags.LOGS) || state.canBeReplaced();
    }
            ...
            for (int zMod = 0; zMod <= size; ) {
                ...
                if (distSq > maxDistSq) {
                    ...
                    do {
                        ...
                        BlockState state = level.getBlockState(placementPos);
                        Block block = state.getBlock();
                        BlockState blockToSet = leaves;
                        if (this.genLightsAndVines) {
                            float randFloat = rand.nextFloat();
                            if (randFloat < 0.03f && level.getBlockState(placementPos.below()).isAir()) {
                                if (this.lights != null && randFloat < 0.02f) blockToSet = lights;
                                else if (this.vines != null)
                                    this.genVines(level, x, y, z, rand.nextInt(this.height / 3));
                            }
                        }
                        if (safeGrowth ? canReplaceWithLeaves(state, level, placementPos) :
                                block != Blocks.BEDROCK.defaultBlockState().getBlock()) {
                            this.setBlockAndNotifyAdequately(level, x, y, z, blockToSet);
                        }
                        if (t == 1) break;
                        t = 1;
                    } while (true);
    ...
    private void placeBlockLine(int[] par1, int[] par2, BlockState logBS, BlockState woodBS) {
        ...
        if (var4[var6] != 0) {
            ...
            for (int var15 = 0; var15 != var16; var15 += var9) {
                ...
                this.setBlockAndNotifyAdequately(level, var14[0], var14[1], var14[2], state2);
            }
        }
    }
    ...
    private void generateTrunk(BlockPos base) {
        ...
        level.getChunk(base).setBlockState(base, Blocks.AIR.defaultBlockState(), false);
        for (int i = -trunkSize; i <= trunkSize; i++) {
            ...
            for (int j = -trunkSize; j <= trunkSize; j++) {
                if ((j * j + i * i) * 4 < trunkSize * trunkSize * 5) {
                    ...
                    this.setBlockAndNotifyAdequately(level, topPoint[0], topPoint[1], topPoint[2], wood);
                }
            }
        }
    }
    ...
    private int checkBlockLine(int[] par1, int[] par2) {
        ...
        if (var3[var5] == 0)
            ...
            for (; var14 != var15; var14 += var8) {
                ...
                if (safeGrowth ? !(canReplaceWithLogs(state, level, pos) ||
    ...
    private boolean validTreeLocation() {
        ...
        int newHeight = Math.min(heightLimit + basePos[1], maxHeight - 1) - basePos[1];
        ...
        if (!(saplingState.getBlock() instanceof BushBlock)) {
            return false;
        }
    ...
    public boolean generate(Level level, RandomSource par2Random, BlockPos pos) {
        ...
        if (!this.validTreeLocation())
            ...
            this.generateLeaves();
            long leavesTime = System.currentTimeMillis();
            logDebug("Generated leaves in: " + (leavesTime - nodes) + "ms");
            this.generateLeafNodeBases();
            long bases = System.currentTimeMillis();
            logDebug("Generated bases in: " + (bases - leavesTime) + "ms");
            ...
            leavesTime -= nodes;
            nodes -= time2;
            logDebug(String.format("%s for trunk, %s for leaf nodes, %s for leaves, %s for branches", trunk, nodes, leavesTime, bases));
    ...
    public void setBlockAndNotifyAdequately(Level level, int x, int y, int z, BlockState state) {
        BlockPos pos = new BlockPos(x, y, z);
        if (safeGrowth && !canReplaceWithLogs(level.getBlockState(pos), level, pos)) return;
    ...
    public void updateChunks() {
        ...
        for (LevelChunk chunk : chunksToUpdate) {
            ...
            for (int i = level.getMinSection(); i < level.getMaxSection(); i++) {
